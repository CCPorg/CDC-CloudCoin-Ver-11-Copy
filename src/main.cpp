...
unsigned int nStakeTargetSpacing = 60;			// 30s block spacing
...
const string strMessageMagic = "cloudcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else
    {
        ...
        {
            nSubsidy = 88 * MAX_MINT_PROOF_OF_WORK;
        }
        else if(rand > 2000 && rand < 2031)
        {
            nSubsidy = 18 * MAX_MINT_PROOF_OF_WORK;
        }
        else if(rand > 8000 && rand < 8101)
        {
            nSubsidy = 8 * MAX_MINT_PROOF_OF_WORK;
        }


        // Subsidy is cut in half every 8000000 blocks,
        nSubsidy >>= (nHeight / 8000000); // cloudcoin
    }
...
static const int64 nTargetSpacingWorkMax = 12 * nStakeTargetSpacing; // 12 mins
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // CDC v1.1 : set fork point at approximately 2 weeks past latest block (271093) at time this bug was fixed.
    // this is done as a future fork so that if the network happens to hit the negative retarget bug again, original v1.0 and updated v1.1 clients will still communicate with each other.
    // v1.1 clients will effectively hard fork if there's a neg retarget at any time past block 291253. if that never happens, neither will the hard fork, and v1.1 and v1.0 will play nicely.
    if (fTestNet || (nHeight > 291253)) nActualSpacing = max(nActualSpacing, (int64)0);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal cloudcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in cloudcoin
    bool fStrictPayToScriptHash = true; // Always active in cloudcoin
    ...
    // ppcoin: fees are not collected by miners as in cloudcoin
...
bool CBlock::AcceptBlock()
{
    ...
    // CDC v1.1 : check for peer providing block from incorrect fork (PoW not possible due to negative retarget bug)
    if (nHeight == 221626 && hash != uint256("0x000000132b720a686e955dde58f04794005fa83e09df472d0878a6d0625e7b98"))
        return DoS(100, error("AcceptBlock() : peer offered block from retired fork"));
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "cloudcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "NY mayoral candidate Anthony Weiner admits more lewd photos";
        CTransaction txNew;
		...
		txNew.strTxComment = "text:cloudcoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0xf406f8ad709a908bd59aba2181a941413104d1b5d812df9af74445e2e57c0546"));
    ...
    // ppcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        ...
        // CDC v1.1 : ensure last checkpoint is recognised
        if (!fTestNet && Checkpoints::hashSyncCheckpoint != uint256("0x00000000550352cf607aea028e605fe2702fe3b11ed59da0f742c1191f924eba")) {
            printf("LoadBlockIndex() : resetting sync checkpoint\n");
...
unsigned char pchMessageStart[4] = { 0xe4, 0xe8, 0xdb, 0xfd };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("cloudcoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("cloudcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("cloudcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("cloudcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadcloudcoinMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void cloudcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running cloudcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadcloudcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        cloudcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadcloudcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadcloudcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadcloudcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d cloudcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadcloudcoinMiner, pwallet))
                printf("Error: NewThread(ThreadcloudcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
